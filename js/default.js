 ...
 * header menu
 */
$(function(){var b=false,c,d=function(){b=false;$("#header_menu>li>ul").hide()},a=function(e){b=true;$(e).children("ul").show()};$("#header_menu>li").click(function(){b?d():a(this)}).mouseover(function(){if(b){if(typeof c!="undefined"){clearTimeout(c)}$("#header_menu>li>ul").hide();a(this)}}).mouseout(function(){c=setTimeout(function(){d()},100)});$("#admin_auth").hide();$("#admin_auth").children('input:password[name="pass"],button').blur(function(){c=setTimeout(function(){d();$("#admin_auth").hide()},100)}).focus(function(){if(typeof c!="undefined"){clearTimeout(c)}});$("#page_delete,#page_lock").click(function(){var f=$(this).attr("id").substr(5);var e=$("#admin_auth");$(e).children("button").removeAttr("disabled");$(e).show().children('input:password[name="pass"]').removeAttr("disabled").val("").focus();$(e).children('input:hidden[name="mode"]').val(f)});$("#admin_auth").children("button").click(function(){var h=$("#admin_auth");var f=$(h).children('input:hidden[name="pid"]').val();var g=$(h).children('input:password[name="pass"]').attr("disabled","disabled").val();var i=$(h).children('input:hidden[name="mode"]').val();$(this).attr("disabled","disabled");var e=document.createElement("img");$(e).attr({src:"/img/ajax-loader.gif",alt:"loading"}).appendTo(h);$.ajax({url:"/action.php?cmd=page"+i,type:"POST",data:{pages:f,pass:g},dataType:"text",beforeSend:function(){},success:function(k,j){switch(k){case"err:pass":alert("管理パスワードが違います。");break;case"success":if(i=="lock"){alert("ロックを変更しました。\n設定を反映する為、リロードします。");location.reload()}else{if(i=="delete"){alert("ページを削除しました。");location.href="/"}}break;default:alert(k)}},error:function(k,j){},complete:function(){d();$("#admin_auth").hide();$(h).children("img").remove()}})});d()});function editorSetting(a){if(confirm("保存されていない編集中のデータは消えてしまいますがよろしいですか？")){a=(a)?true:false;$.cookie("editor",a);window.location.reload()}else{return false}};
...
$(document).ready(function(){
	...
	$("p.area_close").next().hide();

	$('p.area_close,p.area_open').click(function() {
		$(this).toggleClass("area_open").toggleClass("area_close").next().slideToggle("fast");
	});


	$("span.area_close").next().hide();
	
	$("span.area_open,span.area_close").click(function(){
		$(this).toggleClass("area_open").toggleClass("area_close").next().toggle();
	});
...
$(function() {
	function scrollToElement(id){
		//location.hash="#"+id;
		var t=$('body').find('#'+id);
		var top=parseInt(t.position().top) - 30;
	...
	if(location.hash){
		...
		scrollToElement(location.hash.match(/#(.*)/)[1]);
		});
	}
	...
	$('input:text[name="body"]').blur(function() {
	  var text = $.trim($(this).val());
	  $(this).val(text);
	  if( text.length == 0 ) $(this).addClass('tipped').val('コメント');
	});

	$('form.user_comment').submit(function() {
	  var messageString = 'コメントを入力してください。（1～150文字）';
	  var $name = $(this).find('input:text[name="name"]');
	  var $body = $(this).find('input:text[name="body"]');
	  var $message = $(this).find('p.user_comment_error');
	  if( $message.length == 0 ) {
	    $message = $('<p>').addClass('user_comment_error');
	    $(this).find('p.user_comment_message:last').after($message)
	  }
	  $message.hide();
	  
	  if( $body.hasClass('tipped') ) {
	    $message.text(messageString).fadeIn();
	    return false;
	  } else if( $body.val().length > 150 ) {
	    $message.text(messageString).fadeIn();
	    return false;
	  } else {
	    if( $name.hasClass('tipped') ) {
	      $name.val('');
	    } else {
...
$(document).ready(function(){
	var nves = new Array();
	var exists = false;
	$("div.nicovideo_source embed").each(function(){
		nves.push($(document.createElement("embed")).attr({
			type : $(this).attr("type"),
			id : $(this).attr("id"),
			name : $(this).attr("name"),
			src : $(this).attr("src"),
			...
			allowscriptaccess : $(this).attr("allowscriptaccess"),
	...
	if(!exists){
		$("div.nicovideo_source object").each(function(){
			var nvo = document.createElement("object");
			var nvp = null;
			$(this).children("param").each(function(){
				nvp = document.createElement("param");
				nvp.setAttribute("name", $(this).attr("name"));
				nvp.setAttribute("value", $(this).attr("value"));
				nvo.appendChild(nvp);
			});
			nvp = document.createElement("param");
			nvp.setAttribute("name", "wmode");
			nvp.setAttribute("value", "transparent");
			nvo.appendChild(nvp);
			nvo.setAttribute("name", $(this).attr("name"));
			nvo.setAttribute("width", $(this).attr("width"));
			nvo.setAttribute("height", $(this).attr("height"));
			nvo.setAttribute("id", $(this).attr("id"));
			nvo.setAttribute("classid", $(this).attr("classid"));
			nves.push($(nvo));
			exists = true;
		});
	}
	if(exists){
		$("div.nicovideo_view").each(function(){
			$(this).append(nves.shift());
			return nves.length != 0;
		});
		$("div.nicovideo_source embed").remove();
	}
});
...
$(function() {
	$('<li>').addClass('social_menu').append(
		$('<a>').attr(
			{
				href: "https://twitter.com/share?ref_src=twsrc%5Etfw"
			}
		).text("Tweet").addClass("twitter-share-button").data("text", encodeURIComponent($('title').text() + ' ' + location.origin + location.pathname + ' #FC2WIKI')).data("show-count", "false")
	).appendTo('#header_menu');

	$('<script async src="https://platform.twitter.com/widgets.js" charset="utf-8"></script>').appendTo(document.getElementsByTagName("head")[0]);
});


// facebook
$(function() {
	var src = 'https://www.facebook.com/plugins/like.php';
	var setting = {
		href: location.href,
		width: 153,
	...
	for(var i in setting) {
		array.push(i+'='+encodeURIComponent(setting[i]));
	}
	src+= '?'+array.join('&amp;');
	
	var $iframe = $('<iframe src="'+src+'" width="153" height="21" style="border:none; overflow:hidden;" scrolling="no" frameborder="0" allowTransparency="true"></iframe>');
